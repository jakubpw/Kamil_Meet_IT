...
int C(int l, int r){ 
    return query(root[rt[r]], 1, N, l, r);
}

void opt(int l, int r, int warstwa, int lb, int rb){
    if(l>r) return;
    int optk, res = 0, m = (l+r)/2;
    //cout << "Zasieg:" << lb << " " << rb << '\n';
    //cout << "Mid: " << m << "\n";
    for(int k = lb; k <= min(m, rb); k++){
        //int mc = middle[m][k-start[m]];
    ...
    //cout << '\n';
    dp[m][warstwa] = res;
    opt(l, m-1, warstwa, lb, optk);
    opt(m+1, r, warstwa, optk, rb);
}
